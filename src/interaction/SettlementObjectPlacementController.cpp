    ...
    )
{
    ...
    if (lockedFootprint_)
    {
        if (choosingDoor())
        {
            ...
            {
                const auto outside = outsideDoor(*lockedFootprint_, *position);
        ...
        return commitFootprint(*definition, *lockedFootprint_, settlementMap);
    }
    ...
    if (definition->selectionMode == SettlementFootprintSelectionMode::Fixed)
    {
...
) noexcept
{
    ...
    if (definition->id != SettlementObjectTypes::Road && footprint->width < 3 &&
        footprint->height < 3)
    {
        return false;
    }
...
std::optional<SettlementObjectFootprint> SettlementObjectPlacementController::
    ...
    currentFootprint() const noexcept
{
    ...
    const std::int32_t bottom = std::max(dragStart_->y, hoveredPosition_->y);
...
bool SettlementObjectPlacementController::choosingDoor() const noexcept
{
    ...
    return lockedFootprint_ && d && d->id != SettlementObjectTypes::Road &&
           !selectedDoor_;
}
    ...
    visibleDoor() const noexcept
{
    ...
    if (!d || !f || d->id == SettlementObjectTypes::Road)
        return std::nullopt;
