...
namespace Paladin
{
    ...
    {
        constexpr double gameMinutesPerYear = 365.0 * 24.0 * 60.0;
             ...
             step.settlementSteps)
        {
            ...
            SettlementPopulation& population = state.population();
            ...
            const double sustainableSupplyRatio =
                state.economy().populationSustainableSupplyRatio();
            ...
            const double surplusBirthsPerPerson =
                std::clamp(sustainableSupplyRatio - 1.0, 0.0, 1.0) *
                rates.maximumAnnualSurplusBirthsPerPerson;

            const double annualNaturalChange =
                static_cast<double>(population.residents()) *
                (rates.annualBirthsPerPerson + surplusBirthsPerPerson -
                 rates.annualDeathsPerPerson - shortageDeathsPerPerson);
            ...
            const double projectedChange =
                (annualNaturalChange + rates.annualNetMigration) * elapsedYears;
