    ...
    for (std::size_t i = planned.pathIndex; i < planned.path.size(); ++i)
    {
        cost +=
            citizens.navigation_
                .stepCost(map, from, planned.path[i], citizens.movementPolicy);
        from = planned.path[i];
    }
...
)
{
    if (c.breakTaken || !c.workplaceId || minute < c.breakDue ||
        !policy.isWorkTime(minute) || c.carriedAmount > 0 || !c.path.empty() ||
...
)
{
    ...
    const auto random = GenerationNoise::mix(c.id.value() ^ ++c.choiceSequence);
...
)
{
    const auto available = [&](const SettlementCitizen& person)
    {
        if (person.health <= 0 || person.hunger >= person.foodSeekHunger ||
            person.insideHome || !person.path.empty() || person.carriedAmount ||
            minute < person.nextSocialMinute)
            return false;
        if (person.homeId && shouldSleep(person, minute))
            return false;
    ...
    for (auto& other : citizens.citizens_)
    {
        if (other.id == c.id || !available(other) ||
            ...
            continue;
        auto planned = c;
        if (!route(map, citizens, planned, {other.tilePosition, 1, 1}, false))
            continue;
        ...
        c.nextSocialMinute = other.nextSocialMinute = minute + duration + 60;
