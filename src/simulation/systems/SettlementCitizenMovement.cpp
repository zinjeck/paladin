...
)
{
    ...
    for (auto& citizen : citizens_)
    {
        ...
        while (citizen.pathIndex < citizen.path.size())
        {
            ...
            const auto* home =
                map.objectState().completedObject(citizen.homeId);
            ...
            citizen.stepDuration =
                navigation_
                    .stepCost(map, citizen.tilePosition, next, movementPolicy);
            const double used =
                std::min(travel, citizen.stepDuration - citizen.stepProgress);
            citizen.stepProgress += used;
            travel -= used;
            if (citizen.stepProgress + 1e-10 < citizen.stepDuration)
                break;
            citizen.tilePosition = next;
            citizen.insideHome = home && home->footprint.contains(next);
            citizen.stepProgress = 0;
            ++citizen.pathIndex;
            if (travel <= 0)
                break;
    ...
    for (std::size_t scan = 0; scan < count; ++scan)
    {
        ...
        if (!map.grid().isValidPosition(citizen.tilePosition) ||
            citizen.activity != CitizenActivity::Idle ||
            citizen.task.kind != CitizenTaskKind::None || !citizen.path.empty())
            continue;
        ...
        ++requests;
        auto path =
            navigation_.findPath(map, citizen.tilePosition, goal, searchPolicy);
