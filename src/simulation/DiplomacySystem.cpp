...
namespace Paladin
{
    namespace
    {
        ...
        {
            const auto* a = world.realm(observer); const auto* b = world.realm(target);
            if (!a || !b) return 0;
            const auto& x = a->ruler.personality; const auto& y = b->ruler.personality;
            const double similarity = 1 - (std::abs(x.militarism.first()-y.militarism.first()) +
                std::abs(x.authoritarianism.first()-y.authoritarianism.first()) +
                std::abs(x.elitism.first()-y.elitism.first())) / 3;
            const int affinity = a->primaryCultureId() && a->primaryCultureId()==b->primaryCultureId() ? 15 : 0;
            return std::clamp(int(std::lround(24*similarity - 20*x.isolationism.first() - 7)) + affinity, -100, 100);
        }
    }
    double DiplomacySystem::distance(const World& world, RealmId a, RealmId b)
    {
        double result = 3.141592653589793;
        if (!world.realm(a) || !world.realm(b)) return result;
        for (const auto& x : world.settlements()) if (x.ownerRealmId()==a)
            for (const auto& y : world.settlements()) if (y.ownerRealmId()==b)
                result = std::min(result, geographicDistance(x.position(), y.position(), world.grid().width(), world.grid().height()));
        return result;
    }
    ...
    int DiplomacySystem::opinion(const World& world, RealmId observer, RealmId target)
    {
        if (observer==target && observer) return 100;
    ...
    Money DiplomacySystem::suggestedGift(const World& world, RealmId actor, RealmId target)
    {
        const auto* a=world.realm(actor); const auto* b=world.realm(target);
        if (!a || !b || !a->treasury || actor==target || a->treasury->balance<=0) return 0;
        ...
        const long double size=std::sqrt(static_cast<long double>(population(world,actor)) * population(world,target));
                                           ...
                                           DiplomaticAction action, Money gift)
    {
        ...
        if (!inRange(world,actor,target)) return DiplomaticResult::OutOfRange;
        ...
        { relation.firstOpinion=initialOpinion(world,actor,target); relation.secondOpinion=initialOpinion(world,target,actor); }
        const auto changeOpinion=[&](RealmId observer,int amount)
        {
            auto& value=observer==relation.first?relation.firstOpinion:relation.secondOpinion;
        ...
        if (action == DiplomaticAction::Gift)
        {
            ...
            const Money expected=std::max<Money>(1,suggestedGift(world,actor,target));
            const int gratitude=int(std::clamp(10.L*static_cast<long double>(gift)/expected,1.L,25.L));
        ...
        case DiplomaticAction::Alliance:
            ...
            relation.allied = true; changeOpinion(actor,15); changeOpinion(target,15); break;
        case DiplomaticAction::Trade:
            if (relation.atWar) return DiplomaticResult::AtWar;
            if (relation.trading) return DiplomaticResult::Already;
            relation.trading = true; changeOpinion(actor,10); changeOpinion(target,10); break;
        case DiplomaticAction::Tribute:
            if (relation.overlord == actor && relation.tributary == target)
            { relation.overlord = {}; relation.tributary = {}; changeOpinion(target,20); break; }
            if (relation.atWar) return DiplomaticResult::AtWar;
            ...
            relation.overlord = actor; relation.tributary = target; changeOpinion(target,-30); break;
        case DiplomaticAction::War:
            ...
            relation.overlord = relation.tributary = {}; relation.firstOpinion=relation.secondOpinion=-100; break;
        case DiplomaticAction::Peace:
            if (!relation.atWar) return DiplomaticResult::Already;
            relation.atWar = false; changeOpinion(actor,25); changeOpinion(target,25); break;
    ...
    std::string_view diplomaticResultText(DiplomaticResult result) noexcept
    {
        switch (result)
        {
        case DiplomaticResult::OutOfRange: return "Outside diplomatic range (45 degrees).";
