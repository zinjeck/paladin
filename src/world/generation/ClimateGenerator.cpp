...
namespace Paladin
{
    namespace
    {
        ...
        std::vector<std::int32_t> distanceToWater(const WorldGrid& grid)
        {
            ...
            for (std::int32_t y = 0; y < grid.height(); ++y)
            {
                ...
                {
                    const WorldTile* tile = grid.tile({x, y});
                    ...
                    const std::size_t index = tileIndex(x, y, grid.width());
            ...
            {
                const std::size_t currentIndex = openTiles.front();

                openTiles.pop_front();

                const auto currentX = static_cast<std::int32_t>(
                    currentIndex % static_cast<std::size_t>(grid.width())
                );

                const auto currentY = static_cast<std::int32_t>(
                    currentIndex / static_cast<std::size_t>(grid.width())
                );

                const std::int32_t nextDistance = distances[currentIndex] + 1;
                ...
                {
                    const std::int32_t neighborX = currentX + offset[0];

                    const std::int32_t neighborY = currentY + offset[1];
                    ...
                    const std::size_t neighborIndex =
                        tileIndex(neighborX, neighborY, grid.width());
    ...
    {
        const std::vector<std::int32_t> waterDistances = distanceToWater(grid);
        ...
        for (std::int32_t y = 0; y < grid.height(); ++y)
        {
            ...
            for (std::int32_t x = 0; x < grid.width(); ++x)
            {
                ...
                const double landElevation = std::max(
                    0.0,
                ...
                const double temperatureNoise = GenerationNoise::fractal(
                    static_cast<double>(x) * 0.018,
                    static_cast<double>(y) * 0.018,
                    settings.seed ^ 0x7C15'1A2B'3C4D'5E6FULL,
                    3,
                    0.5,
                    2.0
                );
                ...
                const double rainfallNoise =
                    GenerationNoise::fractal(
                        ...
                        settings.seed ^ 0x4211'3B8D'6F20'9AC5ULL,
                        4,
                        0.52,
                        2.0
                    ) * 0.5 +
                ...
                const double coastalMoisture = std::exp(
                    -static_cast<double>(waterDistances[index]) / 28.0
                );

                const double rainfall =
                    0.10 + rainfallNoise * 0.48 + coastalMoisture * 0.27 +
                    tile->temperature.value() * 0.10 - landElevation * 0.12;

                tile->rainfall = Rainfall{static_cast<float>(rainfall)};
            }
        }
    }
} // namespace Paladin
