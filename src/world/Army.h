...
namespace Paladin
{
    ...
    public:
        ...
        // A station is current presence, never a permanent recruitment-home
        // link.
        SettlementId stationedSettlementId() const noexcept
        {
            return station_;
        }
        SettlementId garrisonSettlementId() const noexcept
        {
            return garrison_;
        }
        bool garrisoned() const noexcept
        {
            return bool(garrison_);
        }
        std::span<const SoldierId> soldiers() const noexcept
        {
            return soldiers_;
        }
        std::size_t soldierCount() const noexcept
        {
            return soldiers_.size();
        }
        int rations() const noexcept
        {
            return rations_;
        }
        ArmyId attackTarget() const noexcept
        {
            return attackTarget_;
        }
        ArmyId engagedOpponent() const noexcept
        {
            return engagedOpponent_;
        }
        bool moving() const noexcept
        {
            return routeIndex_ < route_.size();
        }
        ...
        double currentStepMinutes() const noexcept
        {
            ...
            return minutesPerTile_ *
                   std::hypot(dx, double(route_[routeIndex_].y - position_.y));
        }
        double marchDistance() const noexcept
        {
            return double(routeIndex_) +
                   (moving() ? stepMinutes_ / currentStepMinutes() : 0.0);
        }
        ...
        double visualX() const noexcept
        {
            ...
            return position_.x + dx * std::clamp(
                                          stepMinutes_ / currentStepMinutes(),
                                          0.0,
                                          1.0
                                      );
        }
        ...
        {
            return moving() ? position_.y +
                                  (route_[routeIndex_].y - position_.y) *
                                      std::clamp(
                                          stepMinutes_ / currentStepMinutes(),
                                          0.0,
                                          1.0
