...
double roll(std::uint64_t seed, int x, int y, std::uint64_t salt)
{
    return (GenerationNoise::mix(
                seed ^ (std::uint64_t(x) << 32) ^ std::uint64_t(y) ^ salt
            ) >>
            11) *
           0x1.0p-53;
}
...
SettlementNaturalFeatures::SettlementNaturalFeatures(int width, int height) :
    ...
    versions_(
        std::size_t(chunkColumns_) * ((height + ChunkSide - 1) / ChunkSide),
        1
    )
{
}
...
{
    const int left = std::max(0, f.topLeft.x), top = std::max(0, f.topLeft.y);
        ...
        for (int cx = left / ChunkSide; cx * ChunkSide < right; ++cx)
        {
            ...
            if (left <= x0 && top <= y0 && right >= x1 && bottom >= y1)
                ...
                for (int y = std::max(top, y0); y < std::min(bottom, y1); ++y)
                    for (int x = std::max(left, x0); x < std::min(right, x1);
                         ++x)
                        count += at({x, y}).kind != NaturalFeatureKind::None;
        }
    return count;
}
} // namespace Paladin
