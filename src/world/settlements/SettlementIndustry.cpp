...
namespace Paladin
{
    ...
    )
    {
        ...
        if (!object || !isIndustry(object->objectTypeId))
        {
            return false;
        }
        if (workers <= 0 || !std::isfinite(minute) || !std::isfinite(elapsed) ||
            elapsed <= 0)
        {
            return true;
        }
        ...
        const int target =
            std::max(1, std::min({60, workers * 8, capacity / 2}));
        const auto buy =
            [&](std::string_view resource, int requested, bool depotOnly)
        {
                 ...
                 ++marketFallback)
            {
                for (const auto& candidate : map.logistics.inventories())
                {
                    ...
                    if (candidate.id == destinationId ||
                        candidate.kind == InventoryKind::Construction ||
                        candidate.kind == InventoryKind::Home)
                    {
                        continue;
                    }
                    const auto* seller =
                        map.objectState().completedObject(candidate.objectId);
                    if (depotOnly && candidate.kind != InventoryKind::Market &&
                        (!seller || seller->objectTypeId !=
                                        SettlementObjectTypes::ArmySupplyDepot))
                    {
                        continue;
                    }
                    if (!map.logistics
                             .mayExport(map.objectState(), candidate, resource))
                    {
                        continue;
                    }
                    ...
                    if (amount <= 0 || !map.commerce.buyGoods(
                                           source,
                                           destination,
                                           amount,
                                           nullptr,
                    ...
                    const int moved = map.logistics.moveAvailable(
                        source.id,
                        destinationId,
                        resource,
                        amount
                    ...
                    map.commerce
                        .recordFlow(source.id, destinationId, resource, moved);
                    wanted -= moved;
                }
            }
        };
        if (type == SettlementObjectTypes::Barracks)
        {
            buy(SettlementResourceTypes::Rations,
                std::max(
                    0,
                    std::min(capacity, workers * 6) -
                        inventory->amount(SettlementResourceTypes::Rations)
                ),
                true);
            return true;
        }
        ...
        {
            buy(SettlementResourceTypes::Wheat,
                std::max(
                    0,
                    target - inventory->amount(SettlementResourceTypes::Wheat)
                ),
                false);
        }
        if (type == SettlementObjectTypes::ArmySupplyDepot)
        {
                 ...
                 SettlementResourceCatalog::definitions())
            {
                ...
                {
                    food += inventory->amount(definition.id);
                }
            }
        ...
        {
            const int ready =
                map.objectState().prepareGrainHarvest(objectId, minute);
            const int room = map.logistics.receivable(
                destinationId,
        ...
        const auto output = type == SettlementObjectTypes::Bakery
                                ? SettlementResourceTypes::Bread
                                : SettlementResourceTypes::Rations;
        std::string input;
        if (type == SettlementObjectTypes::Bakery)
        {
            input = SettlementResourceTypes::Wheat;
        }
        else
        {
            for (const auto& definition :
            ...
            {
                if (definition.edible && !definition.emergencyOnly &&
                    map.logistics.available(destinationId, definition.id) > 0)
                {
                    input = definition.id;
                    break;
                }
        ...
        if (input.empty() ||
            map.logistics.available(destinationId, input) <= 0 ||
            inventory->amount(output) >= target)
        {
            return true;
        }
        const int amount = std::min(
            target - inventory->amount(output),
            int(std::min(
                1000000.0,
                map.objectState()
                    .accrueProduction(objectId, elapsed * workers / 30.0)
            ))
        );
        const int produced =
            map.logistics.convert(destinationId, input, output, amount, minute);
        if (produced > 0)
        {
    ...
    )
    {
        ...
        const double start = policy.shiftStartMinute,
                     end = policy.shiftEndMinute;
        ...
        double cursor = minute + fullDays * 1440.0,
               remainder = elapsed - fullDays * 1440.0;
        while (remainder > 1e-9)
        {
            ...
            workMinutes += std::max(
                0.0,
                std::min(local + dt, end) - std::max(local, start)
            );
            cursor += dt;
            remainder -= dt;
        }
        ...
        {
            if (!work.operational || !isIndustry(work.objectTypeId) ||
            ...
            produceIndustry(
                map,
                work.objectId,
                int(map.employment().employed(work.id, citizens)),
                minute + elapsed,
                workMinutes
            );
        }
