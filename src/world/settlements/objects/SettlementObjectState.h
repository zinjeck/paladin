...
class SettlementObjectState
{
  public:
    ...
    {
        return position.x < 0 || position.y < 0 || position.x >= mapWidth_ ||
  ...
  private:
    ...
    void removeInfrastructureWithin(const SettlementObjectFootprint& footprint);
    ...
    void rebuildPlacementPrefixCache(const SettlementGrid& grid) const noexcept;
    ...
    mutable std::unordered_map<SettlementObjectId, std::size_t, StrongIdHash>
        objectIndex_;
    mutable std::unordered_map<ConstructionSiteId, std::size_t, StrongIdHash>
        siteIndex_;
