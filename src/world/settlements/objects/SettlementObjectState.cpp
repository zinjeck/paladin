...
{
    return left.topLeft.x < right.topLeft.x + right.width &&
...
)
{
    ...
    deliveries.reserve(definition.constructionResourceCosts.size());
...
)
{
    ...
    for (const auto& site : constructionSites_)
    {
        ...
        const int right = f.topLeft.x + f.width,
    ...
    // Rebuild both layers so an overlapping road survives a cancelled
    // structure.
    std::fill(
...
) const noexcept
{
    return position.x >= topLeft.x && position.y >= topLeft.y &&
           position.x < topLeft.x + width && position.y < topLeft.y + height;
}
...
SettlementObjectState::SettlementObjectState(
    ...
    structureOccupiedTiles_(
        mapWidth > 0 && mapHeight > 0 ? static_cast<std::size_t>(mapWidth) *
...
{
    if (definition.placementLayer == SettlementObjectPlacementLayer::Structure)
    {
        for (const auto& object : completedObjects_)
        {
            if (object.door &&
                footprint.contains(outsideDoor(object.footprint, *object.door)))
                return false;
        }
        for (const auto& site : constructionSites_)
        {
            if (site.door &&
                footprint.contains(outsideDoor(site.footprint, *site.door)))
                return false;
        }
    }
    ...
    return evaluation.footprintAllowed &&
           (definition.allowsPartialPlacement
                ? evaluation.buildableTileCount > 0
                : evaluation.blockedTileCount == 0);
}

SettlementPlacementAreaEvaluation SettlementObjectState::evaluatePlacementArea(
    const SettlementGrid& grid,
    const SettlementObjectDefinition& definition,
    const SettlementObjectFootprint& footprint
) const noexcept
{
    if (grid.width() != mapWidth_ || grid.height() != mapHeight_ ||
        !footprintSizeIsAllowed(definition, footprint) ||
        (definition.uniquePerSettlement && hasObjectType(definition.id)))
    {
        return {};
    }
    ...
    if (!grid.isValidPosition(footprint.topLeft) ||
    ...
    const std::vector<std::uint32_t>& blockedPrefix =
        definition.placementLayer ==
            ...
            ? infrastructureBlockedPrefix_
            : structureBlockedPrefix_;
    const std::size_t blocked = blockedTileCountIn(blockedPrefix, footprint);
...
SettlementTilePlacementStatus SettlementObjectState::placementStatusAt(
    const SettlementGrid& grid,
    const SettlementObjectDefinition& definition,
    SettlementTilePosition position
) const noexcept
...
)
{
    if (door)
    {
        ...
        const auto* tile = grid.tile(outside);
        if (!validDoorTile(footprint, *door) || !tile ||
            tile->terrain == TerrainType::Water ||
            tile->terrain == TerrainType::Mountain || blocksMovement(outside))
            return false;
    }
    ...
    completedObjects_.push_back(
        {objectIds_.generate(), std::string(definition.id), footprint}
    );

    completedObjects_.back().door =
        definition.id == SettlementObjectTypes::Road
            ? std::nullopt
            : std::optional{door.value_or(centerDoor(footprint))};
...
)
{
             ...
             ++y)
        {
                 ...
                 ++x)
            {
                ...
                const bool buildable =
                    x < footprint.topLeft.x + footprint.width &&
                    placementStatusAt(grid, definition, position) ==
    ...
    else
    {
        ...
        constructionSites_.back().door = door.value_or(centerDoor(footprint));
        if (definition.id == SettlementObjectTypes::FishingGrounds)
        {
...
const std::vector<CompletedSettlementObject>& SettlementObjectState::
    completedObjects() const noexcept
{
    return completedObjects_;
}

const std::vector<SettlementConstructionSite>& SettlementObjectState::
    constructionSites() const noexcept
{
    return constructionSites_;
}

void SettlementObjectState::synchronizeIdIndexes() const
{
    if (indexedVersion_ == navigationVersion_)
        return;
    objectIndex_.clear();
    siteIndex_.clear();
    for (std::size_t i = 0; i < completedObjects_.size(); ++i)
        objectIndex_.emplace(completedObjects_[i].id, i);
    for (std::size_t i = 0; i < constructionSites_.size(); ++i)
        siteIndex_.emplace(constructionSites_[i].id, i);
    indexedVersion_ = navigationVersion_;
}
const CompletedSettlementObject* SettlementObjectState::completedObject(
...
) const noexcept
{
    ...
    return it == objectIndex_.end() ? nullptr : &completedObjects_[it->second];
}
const SettlementConstructionSite* SettlementObjectState::constructionSite(
...
) const noexcept
{
    ...
    return it == siteIndex_.end() ? nullptr : &constructionSites_[it->second];
}

const CompletedSettlementObject* SettlementObjectState::completedObjectAt(
    SettlementTilePosition position
) const noexcept
{
...
{
    const auto removeCompleted =
        [this, &footprint](const CompletedSettlementObject& object)
    {
        const SettlementObjectDefinition* definition =
            SettlementObjectCatalog::definition(object.objectTypeId);
    ...
    for (const SettlementConstructionSite& site : constructionSites_)
    {
        ...
        const std::int32_t cutLeft =
            std::max(site.footprint.topLeft.x, footprint.topLeft.x);
        const std::int32_t cutTop =
            std::max(site.footprint.topLeft.y, footprint.topLeft.y);
        const std::int32_t cutRight =
            std::min(siteRight, footprint.topLeft.x + footprint.width);
        const std::int32_t cutBottom =
            std::min(siteBottom, footprint.topLeft.y + footprint.height);
        const std::array<SettlementObjectFootprint, 4> pieces{
            {{site.footprint.topLeft,
             ...
             {{site.footprint.topLeft.x, cutTop},
              ...
              cutBottom - cutTop},
             {{cutRight, cutTop}, siteRight - cutRight, cutBottom - cutTop}}
        };
...
{
    if (cachedPlacementGrid_ == &grid &&
        cachedPlacementVersion_ == navigationVersion_)
    {
        return;
    }

    const std::size_t prefixWidth = static_cast<std::size_t>(mapWidth_) + 1U;
    std::fill(
...
) const noexcept
{
    ...
    return prefix[bottom * stride + right] - prefix[top * stride + right] -
