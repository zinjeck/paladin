...
namespace Paladin
{
    ...
    )
    {
        ...
        for (const auto& site : constructionSites_)
        {
            ...
            const int bottomCut =
                std::min(bottom, area.topLeft.y + area.height);
    ...
    ) const noexcept
    {
            ...
            SettlementObjectPlacementLayer::Structure)
        {
            for (const auto& object : completedObjects_)
            {
                if (object.door &&
                    footprint.contains(
                        outsideDoor(object.footprint, *object.door)
                    ))
                {
                    return false;
                }
    ...
    SettlementPlacementAreaEvaluation SettlementObjectState::
        evaluatePlacementArea(
            const SettlementGrid& grid,
            const SettlementObjectDefinition& definition,
            const SettlementObjectFootprint& footprint
        ) const noexcept
    {
        ...
        const std::size_t blocked =
            blockedTileCountIn(blockedPrefix, footprint);
    ...
    )
    {
        ...
        else
        {
            ...
            if (definition.hasDoor)
            {
                constructionSites_.back().door =
                    door.value_or(centerDoor(footprint));
            }
            if (definition.id == SettlementObjectTypes::FishingGrounds)
            {
    ...
    ) const noexcept
    {
        ...
        return it == objectIndex_.end() ? nullptr
                                        : &completedObjects_[it->second];
    }
    ...
    ) const noexcept
    {
        ...
        return it == siteIndex_.end() ? nullptr
                                      : &constructionSites_[it->second];
    }
    ...
    ) const noexcept
    {
        ...
        const std::size_t right =
            left + static_cast<std::size_t>(footprint.width);
        const std::size_t bottom =
            top + static_cast<std::size_t>(footprint.height);
