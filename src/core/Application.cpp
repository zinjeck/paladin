        ...
        while (running)
        {
            ...
            while (SDL_PollEvent(&event))
            {
                ...
                if (screen_ == Screen::City)
                {
                    ...
                    if (currentMap && currentSettlement)
                    {
                        ...
                        if (settlementInspectionPanel_->editingName())
                        {
                            ...
                            {
                                settlementInspectionPanel_->appendText(
                                    event.text.text
                                );
                                continue;
                            }
                        ...
                        if (employmentPanel_->isOpen())
                        {
                                ...
                                event.button.button == SDL_BUTTON_LEFT)
                            {
                                employmentCapturedPointer_ =
                                    employmentPanel_->pointerPressed(
                                        event.button.x,
                                        event.button.y
                                    );
                                ...
                                employmentCapturedPointer_)
                            {
                                ...
                                if (const auto change =
                                        employmentPanel_->takeWorkDayChange())
                                {
                                ...
                                employmentCapturedPointer_ = false;
                                if (const auto* workplace =
                                        currentMap->employment().workplace(
                                            employmentPanel_
                                                ->takeFocusedWorkplace()
                                        ))
                                {
                                    camera_->setPosition(
                    ...
                    if (event.type == SDL_EVENT_MOUSE_WHEEL)
                    {
                        ...
                        const double multiplier =
                            wheelDelta > 0.0 ? std::pow(1.15, wheelDelta)
                        ...
                        event.button.button == SDL_BUTTON_LEFT)
                    {
                            ...
                            settlementMap)
                        {
                            ...
                            )
                            {
                                ...
                                if (result == SettlementPlacementCommitResult::
                                                  CompletedObject)
                                {
                        ...
                        event.button.button == SDL_BUTTON_LEFT)
                    {
                        ...
                        const CityHudAction action = cityHud_->pointerReleased(
                            event.button.x,
                            event.button.y
                        );
                        ...
                        {
                            const auto section =
                                action == CityHudAction::Population
                                    ? "Population"
                ...
                if (event.type == SDL_EVENT_MOUSE_WHEEL)
                {
                    ...
                    const double multiplier =
                        wheelDelta > 0.0
                            ? std::pow(zoomInPerWheelStep, wheelDelta)
                            : std::pow(zoomOutPerWheelStep, -wheelDelta);
                ...
                if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN &&
                    event.button.button == SDL_BUTTON_LEFT)
                {
                    const bool hudCapturedPointer = worldHud_->pointerPressed(
                        event.button.x,
                        event.button.y
                    );
                        ...
                        settlementPlacementController_->isSelecting())
                    {
                        ...
                        const bool locked =
                            settlementPlacementController_
                                ->lockHoveredSelection(simulation_->world());
                ...
                if (event.type == SDL_EVENT_MOUSE_BUTTON_UP &&
                    event.button.button == SDL_BUTTON_LEFT)
                {
                    const WorldHudAction action = worldHud_->pointerReleased(
                        event.button.x,
                        event.button.y
                    );
                    ...
                    )
                    {
                        ...
                        const Realm* realm =
                            world.realm(simulation_->playerRealmId());
                        const Settlement* capital =
                            realm
                                ? world.settlement(realm->capitalSettlementId())
                                : nullptr;
                        const Culture* culture =
                            realm ? world.culture(realm->primaryCultureId())
                                  : nullptr;
            ...
            if (screen_ == Screen::City)
            {
                ...
                if (settlementMap)
                {
                    auto* owner = simulation_->world().settlement(
                        activeCitySettlementId_
                    );
                    if (owner)
                    {
                        auto& citizens = owner->simulationState().citizens();
                        settlementMap->employment().synchronize(
                            settlementMap->objectState(),
                            citizens
                        );
                        settlementMap->employment().record(
                            simulation_->world().time().totalGameMinutes(),
                            citizens
                        );
                        cityHud_->setSettlementStatus(
                ...
                {
                    employmentPanel_->setRealmWorkDayHours(
                        realm->workDayHours()
                    );
                }
            ...
            else if (lockedPosition)
            {
                const MapColor selectedColor =
                    foundingPanel_->isOpen() ? foundingPanel_->selectedColor()
                                             : MapColor{238, 190, 64};
