        ...
        for (int kind = 0; kind < 5; ++kind)
        {
            ...
            for (int y = 0; y < spriteSide; ++y)
                for (int x = 0; x < spriteSide; ++x)
                {
                    if (!shape(x, y))
                        continue;
                    const bool edge = !shape(x - 1, y) || !shape(x + 1, y) ||
...
) const
{
    ...
    const int lastX = std::clamp(
        int(std::ceil(((renderer.outputWidth() - originX) / tp + 1) / side)),
        0,
        columns
    );
    const int firstY =
    ...
    const int lastY = std::clamp(
        int(std::ceil(((renderer.outputHeight() - originY) / tp + 1) / side)),
        0,
        rows
    );
    const int visibleColumns = lastX - firstX, visibleRows = lastY - firstY;
    const std::size_t visibleCount = std::size_t(visibleColumns) * visibleRows;
            ...
            for (int y = 0; y < side; ++y)
            {
                for (int x = 0; x < side; ++x)
                {
                    ...
                    const auto variation = GenerationNoise::mix(
                        map.generationSeed() ^ (std::uint64_t(tile.x) << 32) ^
                        std::uint64_t(tile.y)
                    );
                        ...
                        for (int sx = 0; sx < spriteSide; ++sx)
                        {
                            const auto mask = spriteMasks()
                                [feature.kind == NaturalFeatureKind::Tree
                                     ? 0
                                     : 1 + variation % 4][sy * spriteSide + sx];
                            if (!mask)
                                continue;
                            const bool edge = mask == 2;
                            const int px =
                                padding + x * pixelsPerTile + sx - 1 + rockX;
                            const int py =
                                padding + y * pixelsPerTile + sy - 1 + rockY;
