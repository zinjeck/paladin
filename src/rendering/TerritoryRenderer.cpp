...
namespace Paladin
{
    namespace
    {
        ...
        struct RealmLabelPlacement
        {
            ...
            double anchorDistanceSquared = std::numeric_limits<double>::max();
        };
        ...
        RenderColor realmColor(const Realm& realm, std::uint8_t alpha) noexcept
        {
        ...
        ) noexcept
        {
            const float safeWidth =
                std::clamp(width, 0.5F, std::max(0.5F, tilePixels * 0.5F));

            switch (edge)
            {
            case TileEdge::Left:
                return {tileX, tileY, safeWidth, tilePixels};
            ...
            case TileEdge::Top:
                return {tileX, tileY, tilePixels, safeWidth};
    ...
    struct TerritoryRendererCache
    {
        ...
        std::vector<RealmLabelPlacement> labelPlacements;
    };
    ...
    ) const
    {
        ...
        const double tilePixelsDouble = metrics.scaledTilePixels(camera.zoom());
        ...
        const std::span<const Realm> realms = world.realms();
        const std::uint64_t currentRealmSignature = realmSignature(realms);

        if (cache_->source != &territory ||
            cache_->territoryRevision != territory.revision() ||
            cache_->realmSignature != currentRealmSignature ||
            cache_->fillAlpha != policy.politicalFillAlpha)
        {
            const bool sourceChanged = cache_->source != &territory;
            ...
            {
                for (const std::size_t tileIndex : cache_->paintedPixelIndices)
                {
                    cache_->overlayPixels[tileIndex] = RenderColor{0, 0, 0, 0};
                }
            ...
            cache_->labelPlacements.resize(realms.size());

            for (std::size_t index = 0; index < realms.size(); ++index)
            {
                cache_->labelPlacements[index].realmId = realms[index].id();
                 ...
                 territory.controlledPositions())
            {
                ...
                cache_->overlayPixels[tileIndex] =
                    realmColor(*realm, policy.politicalFillAlpha);
                ...
                for (const NeighborEdge& neighborEdge : neighborEdges)
                {
                    ...
                    if (territory.controllerAt(neighbor) != controller)
                    {
                ...
                RealmLabelPlacement& placement =
                    cache_->labelPlacements[placementIterator->second];

                ++placement.tileCount;
                placement.sumX += x;
                placement.sumY += y;
                placement.minimumX = std::min(placement.minimumX, x);
                placement.maximumX = std::max(placement.maximumX, x);
            }
                 ...
                 territory.controlledPositions())
            {
                ...
                const std::int32_t y = position.y;
                const auto placementIterator =
                    placementByRealm.find(territory.controllerAt(position));
                ...
                const double centerX = static_cast<double>(placement.sumX) /
                                       static_cast<double>(placement.tileCount);

                const double centerY = static_cast<double>(placement.sumY) /
                                       static_cast<double>(placement.tileCount);
            ...
            if (cache_->politicalOverlay &&
                cache_->politicalOverlay->width() == territory.width() &&
                cache_->politicalOverlay->height() == territory.height())
            {
            ...
            if (!overlayUpdated)
            {
                cache_->politicalOverlay = renderer.createTextureFromPixels(
                    territory.width(),
                    territory.height(),
                    cache_->overlayPixels
                );
        ...
        if (politicalView && cache_->politicalOverlay)
        {
            renderer.drawTexture(
                *cache_->politicalOverlay,
                ...
                static_cast<float>(
                    viewportWidth * 0.5 - camera.tileX() * tilePixelsDouble
                ),
                static_cast<float>(
                    viewportHeight * 0.5 - camera.tileY() * tilePixelsDouble
                ),
                static_cast<float>(
                    static_cast<double>(territory.width()) * tilePixelsDouble
                ),
                static_cast<float>(
                    static_cast<double>(territory.height()) * tilePixelsDouble
                )
            );
        }

        const float tilePixels = static_cast<float>(tilePixelsDouble);
        ...
        std::unordered_map<RealmId, std::vector<RenderRectangle>, StrongIdHash>
            borderRectanglesByRealm;
        ...
        for (const BoundaryEdge& boundary : cache_->boundaryEdges)
        {
            ...
            const float screenY = static_cast<float>(
                viewportHeight * 0.5 +
                (static_cast<double>(boundary.position.y) - camera.tileY()) *
                    tilePixelsDouble
            );

            if (screenX + tilePixels < 0.0F || screenY + tilePixels < 0.0F ||
                screenX > viewportWidth || screenY > viewportHeight)
            {
                continue;
            }

            backdropRectangles.push_back(tileEdgeRectangle(
                screenX,
                screenY,
                tilePixels,
                boundary.edge,
                borderWidth + policy.borderBackdropExtraWidth
            ));

            borderRectanglesByRealm[boundary.realmId].push_back(
                tileEdgeRectangle(
                    screenX,
                    screenY,
                    tilePixels,
        ...
        renderer.fillRectangles(backdropRectangles, backdrop);

        for (const auto& [realmId, rectangles] : borderRectanglesByRealm)
        {
            const Realm* realm = world.realm(realmId);
            ...
            renderer.fillRectangles(rectangles, realmColor(*realm, 255));
        }
        ...
        for (const RealmLabelPlacement& placement : cache_->labelPlacements)
        {
            const Realm* realm = world.realm(placement.realmId);

            if (!realm || placement.tileCount == 0 || realm->name().empty())
            {
                continue;
            }
            ...
            const float pixelSize = std::clamp(
                territoryWidthPixels * policy.labelTerritoryWidthFraction /
            ...
            const float centerX = static_cast<float>(
                viewportWidth * 0.5 + (static_cast<double>(placement.anchor.x) +
                                       0.5 - camera.tileX()) *
                                          tilePixelsDouble
            );
            ...
            const int luminance = 299 * static_cast<int>(mapColor.red) +
