...
namespace Paladin
{
    ...
    ) const
    {
        ...
        if (definition &&
            definition->id == SettlementObjectTypes::FishingGrounds &&
            footprint && !placement.choosingDoor())
        {
            ...
            const float x = ox + f.topLeft.x * size,
                        y = oy + f.topLeft.y * size;
            renderer.fillRectangle(
                x,
                y,
                f.width * size,
            ...
            renderer
                .drawLine(x, y, x + f.width * size, y, {110, 232, 219, 255});
            renderer
                .drawLine(x, y, x, y + f.height * size, {110, 232, 219, 255});
            renderer.drawLine(
                x + f.width * size,
                y,
                x + f.width * size,
