    ...
    )
    {
        ...
        const auto piece = [&](double xx, double yy, bool vertical)
        {
            const auto bounds = p.bounds(
                {xx, yy, 0, vertical ? .16 : 1., vertical ? 1.35 : .42, 0, 1}
            );
            if (!p.visible(bounds))
            {
                return;
            }
            if (!vertical && sprites.placed(
                                 q,
                                 p,
                                 "pasture.fence",
                                 xx + .5,
            ...
            {
                q.submit(
                    {p.bounds({rx, ry, 0, w, h, 0, 0}), color, yy, id, 0, part}
                );
            };
            const RenderColor wood{0x74, 0x51, 0x3F, 255},
                light{0xB7, 0x83, 0x50, 255};
            if (vertical)
            {
        ...
        const int left = std::max(
            0,
            int(p.cameraX - p.screenWidth * .5 / p.tilePixels - x) - 2
        );
        ...
        for (int i = left; i < right; ++i)
        {
            if (i != f.width / 2)
            {
                piece(x + i, y + .08, false);
                piece(x + i, y + f.height - .03, false);
            }
        }
        const int top = std::max(
            0,
            int(p.cameraY - p.screenHeight * .5 / p.tilePixels - y) - 2
        );
        ...
        for (int i = top; i < bottom; ++i)
        {
            if (i != f.height / 2)
            {
                piece(x + .03, y + i + 1, true);
                piece(x + f.width - .14, y + i + 1, true);
            }
        }
    }
} // namespace Paladin
