...
namespace Paladin
{
    ...
    std::size_t DebugConsole::next(std::string_view s, std::size_t p)
    {
        ...
        while (p < s.size() &&
               (static_cast<unsigned char>(s[p]) & 0xc0) == 0x80)
        {
            ++p;
        }
        return p;
    ...
    std::size_t DebugConsole::hitText(float x, float y) const
    {
        ...
        {
            auto q = next(text, p);
            float a = font_.measureWidth(
                std::string_view(text).substr(begin, p - begin)
            );
            float b = font_.measureWidth(
                std::string_view(text).substr(begin, q - begin)
            );
            if (x < left + (a + b) / 2)
            {
                break;
    ...
    bool DebugConsole::handle(const SDL_Event& e)
    {
        ...
        {
            if (statsOpen_ &&
                statsBox_.contains(e.wheel.mouse_x, e.wheel.mouse_y))
            {
        ...
        if (ctrl && (e.key.scancode == SDL_SCANCODE_C ||
        ...
        case SDL_SCANCODE_DOWN:
            ...
            input_ = historyIndex_ < history_.size() ? history_[historyIndex_]
                                                     : draft_;
            cursor_ = anchor_ = input_.size();
            break;
        default:
    ...
    )
    {
        ...
        if (open_)
        {
            ...
            firstLine_ =
                std::max(0, int(outputLines_.size()) - visible - scroll_);
            ...
            if (inputFocus_ && b > a)
            {
                box({inputBox_.x + 6 +
                     ...
                     inputBox_.y + 3,
                     font_.measureWidth(
                         std::string_view(input_).substr(a, b - a)
                     ),
                     24},
                    {100, 110, 129, 255});
            }
        ...
        if (statsOpen_)
        {
            // Deliberate visual exception: only diagnostic stats use
            ...
            {
                box({statsBox_.x + 2, statsBox_.y + 2, 28, 26},
                    {10, 40, 65, 220});
            }
            if (!ui.drawButtonSprite(
                    r,
            ...
            {
                box({statsBox_.x + 34, statsBox_.y + 2, 28, 26},
                    {10, 40, 65, 220});
            }
            font_.drawText(
                r,
