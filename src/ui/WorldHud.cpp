...
{
WorldHud::WorldHud() :
    selectRegionButton_("Select Region"), moveCapitalButton_("Move Capital"),
    renameCapitalButton_("Rename Capital"), editPolityButton_("Edit Polity"),
    playButton_("Play"), backButton_("Back")
{
    playButton_.setEnabled(false);
}

void WorldHud::layout(int viewportWidth, int viewportHeight) noexcept
{
    ...
    selectRegionButton_.setBounds(
        {(static_cast<float>(viewportWidth) - width) * 0.5F,
         16.0F,
         width,
         height}
    );
    ...
    renameCapitalButton_.setBounds(
        {actionRowX + width + gap, 16.0F, width, height}
    );
    editPolityButton_.setBounds(
        {actionRowX + (width + gap) * 2.0F, 16.0F, width, height}
    );

    backButton_.setBounds(
        {0.0F, static_cast<float>(viewportHeight) - height, 140.0F, height}
    );
...
bool WorldHud::pointerPressed(float x, float y) noexcept
{
    ...
    {
        actionCaptured = moveCapitalButton_.pointerPressed(x, y) ||
                         renameCapitalButton_.pointerPressed(x, y) ||
                         editPolityButton_.pointerPressed(x, y);
    }
    else if (!capitalEstablished_ || additionalSelection_)
    {
        actionCaptured = selectRegionButton_.pointerPressed(x, y);
    }

    const bool backCaptured = backButton_.pointerPressed(x, y);

    const bool playCaptured = playButton_.pointerPressed(x, y);

    return actionCaptured || playCaptured || backCaptured;
}

bool WorldHud::containsInteractivePoint(float x, float y) const noexcept
{
    return backButton_.containsPoint(x, y) || playButton_.containsPoint(x, y) ||
           ((!capitalEstablished_ || additionalSelection_) &&
            selectRegionButton_.containsPoint(x, y)) ||
           (capitalEstablished_ && !simulationControlsUnlocked_ &&
            (moveCapitalButton_.containsPoint(x, y) ||
             renameCapitalButton_.containsPoint(x, y) ||
             editPolityButton_.containsPoint(x, y)));
}

WorldHudAction WorldHud::pointerReleased(float x, float y) noexcept
{
    const bool selectRegionClicked = selectRegionButton_.pointerReleased(x, y);

    const bool moveClicked = moveCapitalButton_.pointerReleased(x, y);

    const bool renameClicked = renameCapitalButton_.pointerReleased(x, y);

    const bool editClicked = editPolityButton_.pointerReleased(x, y);

    const bool backClicked = backButton_.pointerReleased(x, y);

    const bool playClicked = playButton_.pointerReleased(x, y);

    if ((!capitalEstablished_ || additionalSelection_) && selectRegionClicked)
    {
        return WorldHudAction::SelectRegion;
    }

    if (capitalEstablished_ && !simulationControlsUnlocked_ && moveClicked)
    {
        return WorldHudAction::MoveCapital;
    }

    if (capitalEstablished_ && !simulationControlsUnlocked_ && renameClicked)
    {
        return WorldHudAction::RenameCapital;
    }

    if (capitalEstablished_ && !simulationControlsUnlocked_ && editClicked)
    {
...
{
    if (!capitalEstablished_ || additionalSelection_)
    {
    ...
    {
        const UiRectangle
            bounds{float(renderer.outputWidth()) * .5F - 95, 16, 190, 44};
        uiRenderer.drawPanel(
            renderer,
            {bounds.x - 155, bounds.y + 50, 500, 32}
        );
        uiRenderer.drawLabel(
            renderer,
            "Mostly land, within 48 tiles of your capital",
            bounds.x - 143,
            bounds.y + 60,
            1.6F
        );
