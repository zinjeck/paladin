...
FoundingPanel::FoundingPanel() :
    ...
    leftButton_("Cancel"), rightButton_("Continue"), pickerDoneButton_("Done")
{
    refreshButtonState();
}
...
bool FoundingPanel::canConfirm() const noexcept
{
    ...
    return step_ == FoundingPanelStep::Capital && canContinueFromPolity() &&
...
FoundingIdentity FoundingPanel::identity() const
{
    const std::string_view originId =
        selectedOriginIndex_ ? startingPolityOrigins[*selectedOriginIndex_].id
                             : std::string_view{};
...
void FoundingPanel::render(Renderer& renderer, const GrayUiRenderer& uiRenderer)
{
    ...
    const auto heading =
        [&](std::string_view text, const UiRectangle& box, float preferredSize)
    {
    ...
    {
        heading(
            mode_ == FoundingPanelMode::EditPolity ? "EDIT YOUR POLITY"
                                                   : "FOUND YOUR POLITY",
            panelBounds_,
            4.4F
        );
    ...
    {
        heading(
            mode_ == FoundingPanelMode::NewSettlement   ? "FOUND NEW SETTLEMENT"
            : mode_ == FoundingPanelMode::RenameCapital ? "RENAME CAPITAL"
                                                        : "FOUND YOUR CAPITAL",
            panelBounds_,
            4.4F
        );
    ...
    if (colorPickerTarget_ != ColorPickerTarget::None)
    {
        ...
        for (std::size_t channel = 0; channel < 3; ++channel)
        {
            ...
            renderer.fillRectangle(
                bounds.x + 2.0F,
                bounds.y + 2.0F,
                (bounds.width - 4.0F) * static_cast<float>(values[channel]) /
                    255.0F,
                bounds.height - 4.0F,
                fills[channel]
            );
        ...
        uiRenderer.drawColorSwatch(
            renderer,
            {pickerBounds_.x + 28.0F, pickerBounds_.y + 264.0F, 72.0F, 42.0F},
            renderColor(color),
            false,
            true
        );
