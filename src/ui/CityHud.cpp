...
) noexcept
{
    const float width =
        text.empty()
            ? 0.0F
            : (static_cast<float>(text.size()) * 6.0F - 1.0F) * pixelSize;
...
void CityHud::layout(int viewportWidth, int viewportHeight) noexcept
{
    ...
    const float rowX = (static_cast<float>(viewportWidth) - rowWidth) * 0.5F;
    ...
    const float minimapSide = std::max(
        0.0F,
        std::min(
            {220.0F, rowX - 8.0F, static_cast<float>(viewportHeight) - 100.0F}
        )
    );
    minimapPanel_ = {
...
bool CityHud::pointerPressed(float x, float y) noexcept
{
    ...
    for (std::size_t index = 0; index < optionButtons_.size(); ++index)
    {
        if (optionIsVisible(index))
        {
            captured = optionButtons_[index].pointerPressed(x, y) || captured;
        }
    }
...
bool CityHud::containsInteractivePoint(float x, float y) const noexcept
{
    ...
    if (worldMode_)
        return cityNamePanel_.contains(x, y) || dayTimePanel_.contains(x, y) ||
    ...
    for (std::size_t index = 0; index < optionButtons_.size(); ++index)
    {
        if (optionIsVisible(index) && optionButtons_[index].containsPoint(x, y))
        {
            return true;
        }
    }
...
CityHudAction CityHud::pointerReleased(float x, float y) noexcept
{
    ...
    for (std::size_t index = 0; index < optionButtons_.size(); ++index)
    {
        if (optionIsVisible(index))
        {
            const bool clicked = optionButtons_[index].pointerReleased(x, y);
...
void CityHud::render(Renderer& renderer, const GrayUiRenderer& uiRenderer) const
{
    ...
    const auto populationLabel =
        ...
        (hasKeep_ ? " " + std::to_string(population_) : "");
    ...
    const float timePixelSize = timeWidthAtPreferredSize > availableTimeWidth
                                    ? preferredTimePixelSize *
                                          availableTimeWidth /
    ...
    uiRenderer.drawLabel(
        ...
        dayTimePanel_.y + (dayTimePanel_.height - 7.0F * timePixelSize) * 0.5F,
        timePixelSize
    );
    ...
    if (worldMode_)
    {
        ...
        const std::string label = "Active Settlement: " + activeSettlementName_;
    ...
    if (goodsOpen_)
    {
        ...
        for (std::size_t i = 0; i < 3; ++i)
        {
            ...
            if (i == 2)
            {
                renderer.fillRectangle(x - 12, y, 24, 22, {90, 90, 90, 255});
                renderer
                    .fillRectangle(x - 8, y + 6, 15, 9, {78, 155, 195, 255});
                renderer
                    .fillRectangle(x - 5, y + 3, 8, 15, {108, 190, 224, 255});
                renderer
                    .fillRectangle(x + 7, y + 4, 4, 13, {70, 135, 175, 255});
                renderer.fillRectangle(x - 7, y + 7, 2, 2, {20, 35, 45, 255});
            }
            ...
            const float size = std::min(
                2.0F,
                (cell.width - 8) / std::max(1.0F, float(label.size()) * 6 - 1)
            );
            uiRenderer.drawLabel(
                renderer,
                label,
    ...
    for (std::size_t index = 0; index < optionButtons_.size(); ++index)
    {
        ...
        const float maximumIconWidth =
            isFoodWorkplace ? bounds.width - 14.0F : (isRoad ? 18.0F : 36.0F);
        const float maximumIconHeight =
            isFoodWorkplace ? bounds.height - 14.0F : (isRoad ? 18.0F : 36.0F);
        const float scale = std::min(
        ...
        const float iconHeight = objectDefinition->visual.iconHeight * scale;
        ...
        const auto labelPixelSize = [&bounds](std::string_view text) noexcept
        {
            ...
            return preferredWidth > bounds.width - 4.0F
                       ? preferredSize * (bounds.width - 4.0F) / preferredWidth
                       : preferredSize;
        };

        const float firstLinePixelSize = labelPixelSize(definition.firstLine);

        uiRenderer.drawLabel(
            renderer,
            definition.firstLine,
            centeredLabelX(bounds, definition.firstLine, firstLinePixelSize),
            firstLineY,
            firstLinePixelSize,
            {250, 250, 250, 255}
        );
...
std::string CityHud::tooltipAt(float x, float y) const
{
    ...
    {
        constexpr std::array
            goods{"Stone", "Lumber", "Fish - food", "Empty", "Empty", "Empty"};
        for (std::size_t i = 0; i < goodsCells_.size(); ++i)
