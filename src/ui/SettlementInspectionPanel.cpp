...
std::string deliveryLabel(const ConstructionResourceDelivery& delivery)
{
    ...
    std::string label =
        definition ? std::string(definition->displayName) : delivery.resourceId;
...
)
{
    ...
    const SettlementCitizen* citizen = controller.selectedCitizen(citizenState);

    const auto* pile = controller.selectedInventory(settlementMap.logistics);
    if (pile && pile->used() <= 0)
    {
        pile = nullptr;
    }
    ...
    if (constructionSite)
    {
        const bool showDeliveries =
            constructionSite->phase != ConstructionSitePhase::UnderConstruction;
        const std::size_t deliveryCount =
            showDeliveries ? constructionSite->resourceDeliveries.size() : 0U;
    ...
    if (citizen)
    {
            ...
            [&](const char* name, double value, float y, bool hunger)
        {
            ...
            const float severity =
                hunger ? std::max(0.0F, (fraction - .5F) * 2) : 1 - fraction;
        ...
        if (citizen->carriedAmount > 0)
        {
            const auto* definition =
                SettlementResourceCatalog::definition(citizen->carriedResource);
            label(
    ...
    {
        const auto employed =
            settlementMap.employment().employed(workplace->id, citizenState);
        grayUiRenderer.drawLabel(
            renderer,
...
bool SettlementInspectionPanel::containsPoint(float x, float y) const noexcept
{
...
) const noexcept
{
    ...
    const float viewportHeight = static_cast<float>(renderer.outputHeight());
